...
-- -- Parsing Issue: Instructor comment changed to user comment
-- -- Current Handling: parses appropriately if changes made to sections not including question numbers, but these changed comments are also written out to AAA_student_comments.out file
-- -- Desired Handling: should parse and evaluate appropriately, with changed comments treated as user comments and written to comments file
...
SELECT TransLocation, CAST( AVG(T1.TransAmount) AS NUMBER(*,2)) AS AvgTrans
FROM Transaction T1
