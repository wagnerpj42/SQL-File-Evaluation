...
public class TestResultSetEqualContentTests extends AbstractTest {
	...
	public void testResultSetEqual() {
		...
		assertEquals(testRSEC.sqlTest(testDAO, creatureAllQuery, creatureAllQuery.toString()), 10);
		// valid queries of same table with different column selects are not equal
		assertEquals(testRSEC.sqlTest(testDAO, creatureAllQuery, creatureOneColQuery.toString()), 0);
		// valid queries of same table but different case are equal
		assertEquals(testRSEC.sqlTest(testDAO, creatureAllQuery, creatureAllQueryLC.toString()), 10);
		// valid queries of same table using * and all columns separately are equal
		assertEquals(testRSEC.sqlTest(testDAO, creatureAllQuery, creatureAllQueryBC.toString()), 10);
		// valid queries of same table with different orderings of columns are equal
		assertEquals(testRSEC.sqlTest(testDAO, creatureAllQueryBC, creatureAllQueryBC2.toString()), 10);
		// valid queries of different tables are not equal
		assertEquals(testRSEC.sqlTest(testDAO, creatureAllQuery, achievementAllQuery.toString()), 0);
		// two null queries are not equal
		assertEquals(testRSEC.sqlTest(testDAO, nullQuery, nullQuery.toString()), 0);
		// null query and good query are not equal 
		assertEquals(testRSEC.sqlTest(testDAO, nullQuery, creatureAllQuery.toString()), 0);
		// non-compiling query and valid query are not equal
		assertEquals(testRSEC.sqlTest(testDAO, badQuery, creatureAllQuery.toString()), 0);
