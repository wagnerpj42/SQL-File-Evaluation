...
public class CondGroupByCountTests extends AbstractTest {
	...
	public void testSqlCond() {
		...
		assertEquals(condGBC.sqlTest(testDAO, groupByQuery, " >= 1"), 10);
		// valid nested query with no group bys
		assertEquals(condGBC.sqlTest(testDAO, nestedQuery, " == 1"), 0);
		// null query has no group bys
		assertEquals(condGBC.sqlTest(testDAO, nullQuery, " >= 1"), 0);
		// bad query with select/improper table has no group bys
		assertEquals(condGBC.sqlTest(testDAO, badQuery, " >= 1"), 0);
		// bad query with garbage has no group bys
		assertEquals(condGBC.sqlTest(testDAO, garbageQuery, " >= 1"), 0);
	}
