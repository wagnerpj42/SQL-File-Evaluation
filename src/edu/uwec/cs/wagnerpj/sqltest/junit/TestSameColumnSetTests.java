...
public class TestSameColumnSetTests extends AbstractTest {
	...
	public void testSameColumnSet() {
		...
		assertEquals(testSCS.sqlTest(testDAO, creatureAllQuery, creatureAllQuery.toString()), 10);
		// queries of same tables with * and columns listed are equal
		assertEquals(testSCS.sqlTest(testDAO, creatureAllQuery, creatureAllQueryBC.toString()), 10);
		// queries of same tables with same columns but in different orders are equal
		assertEquals(testSCS.sqlTest(testDAO, creatureAllQueryBC, creatureAllQueryBC2.toString()), 10);
		// queries of different tables with same number of columns are not equal
		assertEquals(testSCS.sqlTest(testDAO, creatureAllQuery, achievementAllQuery.toString()), 0);
		// queries of same table with different column selects are not equal in column count
		assertEquals(testSCS.sqlTest(testDAO, creatureAllQuery, creatureOneColQuery.toString()), 0);
		// queries of different tables with different column counts are not equal in column count
		assertEquals(testSCS.sqlTest(testDAO, creatureAllQuery, skillAllQuery.toString()), 0);
		// valid table query and null query are not equal in column count
		assertEquals(testSCS.sqlTest(testDAO, creatureAllQuery, nullQuery.toString()), 0);
		// valid table query and bad query are not equal in column count
		assertEquals(testSCS.sqlTest(testDAO, creatureAllQuery, badQuery.toString()), 0);
	}
