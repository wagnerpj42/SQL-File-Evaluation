...
public class CondOrderByCountTests extends AbstractTest {
	...
	public void testSqlCond() {
		...
		assertEquals(condOBC.sqlTest(orderByQuery, " >= 1"), 10);
		// valid nested query with no group bys
		assertEquals(condOBC.sqlTest(nestedQuery, " == 1"), 0);
		// null query has no group bys
		assertEquals(condOBC.sqlTest(nullQuery, " >= 1"), 0);
		// bad query with select/improper table has no group bys
		assertEquals(condOBC.sqlTest(badQuery, " >= 1"), 0);
		// bad query with garbage has no group bys
		assertEquals(condOBC.sqlTest(garbageQuery, " >= 1"), 0);
	}
