...
public class TestResultSetEqualExactlyTests extends AbstractTest {
	...
	public void testResultSetEqual() {
		...
		assertEquals(testRSE.sqlTest(creatureAllQuery, creatureAllQuery.toString()), 10);
		// queries of same table with different column selects are not equal
		assertEquals(testRSE.sqlTest(creatureAllQuery, creatureOneColQuery.toString()), 0);
		// queries of same table but different case are equal
		assertEquals(testRSE.sqlTest(creatureAllQuery, creatureAllQueryLC.toString()), 10);
		// queries of same table using * and all columns separately are equal
		assertEquals(testRSE.sqlTest(creatureAllQuery, creatureAllQueryBC.toString()), 10);
		// valid queries of different tables are not equal
		assertEquals(testRSE.sqlTest(creatureAllQuery, achievementAllQuery.toString()), 0);
		// two null queries are not equal
		assertEquals(testRSE.sqlTest(nullQuery, nullQuery.toString()), 0);
		// null query and good query are not equal 
		assertEquals(testRSE.sqlTest(nullQuery, creatureAllQuery.toString()), 0);
		// non-compiling query and valid query are not equal
		assertEquals(testRSE.sqlTest(badQuery, creatureAllQuery.toString()), 0);
