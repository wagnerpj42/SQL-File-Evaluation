...
public class CondJoinCountTests extends AbstractTest {
	...
	public void testSqlCond() {
		...
		assertEquals(condJC.sqlTest(testDAO, creatureAllQuery, " == 0"), 10);
		// valid nested query with two select/froms has no joins
		assertEquals(condJC.sqlTest(testDAO, nestedQuery, " == 1"), 0);
		// valid query with one JOIN/ON
		assertEquals(condJC.sqlTest(testDAO, joinOneQuery, " == 1"), 10);
		// valid query with one comma join and one JOIN/ON
		assertEquals(condJC.sqlTest(testDAO, joinManyQuery, " == 2"), 10);
		// valid query with table joined to itself with JOIN/ON
		assertEquals(condJC.sqlTest(testDAO, joinTableSelfQuery, " == 1"), 10);
		// null query has no joins
		assertEquals(condJC.sqlTest(testDAO, nullQuery, " >= 1"), 0);
		// bad query with select/improper column list has no joins
		assertEquals(condJC.sqlTest(testDAO, badQuery, " == 0"), 10);
		// bad query with garbage has no joins
		assertEquals(condJC.sqlTest(testDAO, garbageQuery, " >= 1"), 0);
	}
