...
public class CondMinusCountTests extends AbstractTest {
	...
	public void testSqlCond() {
		...
		assertEquals(condMC.sqlTest(testDAO, minusQuery, " == 1"), 10);		
		// valid query displaying count with one group by but no minus
		assertEquals(condMC.sqlTest(testDAO, groupByQuery, " >= 1"), 0);
		// valid nested query with two selects but no minus
		assertEquals(condMC.sqlTest(testDAO, nestedQuery, " == 0"), 10);
		// null query has no minus
		assertEquals(condMC.sqlTest(testDAO, nullQuery, " >= 1"), 0);
		// bad query with select but little else has no minus
		assertEquals(condMC.sqlTest(testDAO, badQuery, " >= 1"), 0);
		// bad query with garbage has no minus
		assertEquals(condMC.sqlTest(testDAO, garbageQuery, " >= 1"), 0);
	}
