...
SELECT A.ACCOPENLOCATION,A.ACCSTATUS FROM ACCOUNT A
...
SELECT A.ACCNUMBER,A.ACCBALANCE,A.ACCTYPE FROM ACCOUNT A
WHERE(A.ACCBALANCE>(SELECT DISTINCT AVG(A.ACCBALANCE) FROM ACCOUNT A
                    ))
...
SELECT T.TRANSLOCATION,CAST(AVG(T.ACCNUMBER) AS NUMBER (*,2)) FROM TRANSACTION T
GROUP BY T.TRANSLOCATION
HAVING AVG(T.TRANSAMOUNT)<(SELECT AVG(T.TRANSAMOUNT) FROM TRANSACTION T1)
...
SELECT C.CUSTID,C.FNAME,C.LNAME,T.TRANSTYPE,MAX(T.TRANSAMOUNT) FROM CUSTOMER C
JOIN ACCOUNT A
