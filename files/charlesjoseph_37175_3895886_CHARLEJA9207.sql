...
WHERE ACCBALANCE >
  ...
  FROM ACCOUNT
  WHERE ACCTYPE = 'checking' OR ACCTYPE = 'savings')
...
SELECT TRANSLOCATION, CAST (AVG(TRANSAMOUNT) AS NUMBER(*,2)) AS AvgAmount
FROM TRANSACTION 
GROUP BY TRANSLOCATION
HAVING AVG(TRANSAMOUNT) < SUM(TRANSAMOUNT)
...
SELECT C.CUSTID, C.FNAME, C.LNAME, MAX(T.TRANSAMOUNT) AS MaxTransaction
FROM TRANSACTION T
JOIN ACCOUNT A
...
GROUP BY CUSTID, FNAME, LNAME, C.CUSTID, C.FNAME, C.LNAME
ORDER BY CUSTID ASC;
...
SELECT C.CUSTID, C.FNAME, C.LNAME, A.ACCSTATUS
FROM ACCOUNT A
