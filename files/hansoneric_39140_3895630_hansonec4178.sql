...
SELECT C.CustID, C.FName, C.LName, A.AccClosedDate
  ...
  JOIN ACCOUNT A ON (C.CustID = A.Customer)
    WHERE LOWER(A.AccOpenLocation) LIKE 'central'
    AND TO_DATE(A.AccClosedDate, 'dd-MON-yyyy') < '01-JAN-2018';
...
SELECT C.CustID, C.FName, C.LName
  FROM CUSTOMER C
  JOIN ACCOUNT A ON(C.CustID = A.Customer)
    WHERE LOWER(A.AccStatus) LIKE 'closed'
    OR LOWER(A.AccStatus) LIKE 'frozen'
