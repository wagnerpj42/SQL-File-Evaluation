...
WHERE AccOpenLocation = 'Central' AND AccClosedDate >= '01-Jan-2018';
...
SELECT  AccOpenLocation, ACCSTATUS, COUNT(ACCSTATUS) AS ave
From Account 
...
SELECT ACCNUMBER
From Account 
WHERE ACCTYPE = 'checking' AND ACCBALANCE > (SELECT AVG(ACCBALANCE)
                                              from ACCOUNT)
;
...
SELECT DISTINCT TransLocation, AVG(TRANSID) AS ave
FROM TRANSACTION
WHERE ave <= (SELECT AVG(TRANSID) FROM TRANSACTION)
